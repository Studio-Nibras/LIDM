...
exports.createNote = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Gagal membuat materi.', error: error.message });
    }
};
...
exports.generateMindMap = async (req, res) => {
    try {
        ...
        if (!lectureId) {
            return res.status(400).json({ message: 'lectureId (ID Workspace) harus diisi!' });
        }
        ...
        if (error) throw error;
        res.status(200).json({ message: 'AI berhasil membuat Mind Map!', mindMap: data });
    } catch (error) {
...
exports.updateMindMap = async (req, res) => {
    try {
        ...
        if (error) throw error;
        res.status(200).json({ message: 'Mind Map berhasil diperbarui!', mindMap: data });
    } catch (error) {
        res.status(500).json({ message: 'Gagal memperbarui Mind Map.', error: error.message });
    }
};

// download mind map dan proses audio 
exports.downloadMindMap = async (req, res) => {
    try { res.status(200).json({ message: "File siap diunduh" }); } catch (e) { res.status(500).json({ message: 'Gagal.' }); }
};
exports.processMicAudio = async (req, res) => {
    try { res.status(201).json({ message: 'Audio mikrofon berhasil direkam!' }); } catch (e) { res.status(500).json({ message: 'Eror.' }); }
};
...
exports.generateQuiz = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Gagal memproses fitur AI.', error: error.message }); 
    }
};
...
exports.submitQuizAnswers = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Gagal memproses jawaban kuis.', error: error.message }); 
    }
};